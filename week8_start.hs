...
transformeer xs [] = [map Verwijder xs]
transformeer (x:xs) (y:ys)
    | x == y    = map (Kopieer x :) (transformeer xs ys)
    | otherwise = 
        map (Vervang x y :) (transformeer xs ys)        -- Vervangen
        ++ map (Verwijder x :) (transformeer xs (y:ys)) -- Verwijderen
        ++ map (Insert y :) (transformeer (x:xs) ys)    -- Invoegen

transformeerKortste :: String -> String -> [[Edit]] 
transformeerKortste xs ys = minimum (map length (transformeer xs ys))