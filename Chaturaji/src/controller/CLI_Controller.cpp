...
CLI_Controller::CLI_Controller(Game &gameModel, CLIView &gameView)
    : gameModel(gameModel), gameView(gameView){
    io = ConsoleIO();
}

void CLI_Controller::askCellProcedure(const std::function<bool(QPoint)>& gameFunc) {
    bool validSelection;
    do{
...
void CLI_Controller::start() {
    setup();
    while(true)
        loop();
}

void CLI_Controller::setup() {
